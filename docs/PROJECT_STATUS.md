...
本輪已新增 `HexagramView` 自訂 View，集中處理六爻卦象繪製，`Ui.hexagramView(...)` 目前作為相容 factory。也已建立 XML component foundations：
...
這些 layout 先作為下一步 XML/ViewBinding 搬遷基礎；核心畫面尚未全量切換。
...
- `StableBetaWorkflowInstrumentedTest`：Espresso workflow tests，啟用 accessibility checks，覆蓋 onboarding → 本機 daily、占卜 → result auto-save → records、紀錄筆記編輯/單筆刪除、學習中心收藏、深色模式切換、JSON/text SAF export contracts 與 profile delete-all。
...
- `./gradlew pixel2Api35DebugAndroidTest` 通過。
...
ViewBinding 已開啟，top bar、record item、settings row、empty state 與 hexagram renderer 已有第一批 foundations；但 bottom nav、完整 form controls，以及 `RecordsFragment` / `ResultFragment` / `ProfileSettingsFragment` 的 XML binding 搬遷尚未完成。
...
- 旋轉後 result 不重複儲存測試。
- records filter state 保留測試。
- SAF 寫入成功內容驗證，目前先覆蓋 ACTION_CREATE_DOCUMENT intent。
- 視覺截圖測試。
...
2. 補旋轉後 result 不重複儲存、records filter state 保留、SAF 寫入內容與更多 delete-all 邊界案例測試。
3. 把 `RecordsFragment`、`ResultFragment`、`ProfileSettingsFragment` 分批 XML/ViewBinding 化，優先使用已建立的 include/item layouts。
