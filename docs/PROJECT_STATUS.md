...
- Question/method/result/hexagram detail 參數以簡單 Bundle / JSON snapshot 傳遞；本階段未導入 Safe Args。
- 啟動時套用 `SettingsStore` 中保存的深色模式，並執行舊紀錄到 Room 的一次性匯入。
...
- `RecordsViewModel`：提供 records LiveData、筆記更新與單筆刪除。
- `ResultViewModel`：處理 result auto-save 與筆記保存。
- `ProfileSettingsViewModel`：提供紀錄匯出與 delete-all。
...
- `RecordRepositoryTest`：覆蓋 Room entity mapper、匯出 JSON、匯出純文字與 legacy JSON parser。
...
- `./gradlew connectedDebugAndroidTest` 已成功建置 app/test APK，但執行階段因目前環境沒有連接實機或 emulator 失敗，錯誤為 `No connected devices!`。
...
1. 評估導入 Safe Args 或明確的 navigation argument contract。
