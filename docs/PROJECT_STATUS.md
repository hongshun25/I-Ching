...
`RecordsFragment`、`ResultFragment`、`ProfileSettingsFragment` 不再直接讀寫紀錄 SharedPreferences JSON，且主要畫面結構已改用 XML/ViewBinding。Fragment 目前只負責 bind state、註冊 listener、呼叫 ViewModel 與 repository-backed helpers。
...
結果頁分享文字/變爻摘要、紀錄卡顯示文字與學習中心收藏按鈕顯示已抽到 `ui/presentation/` 的純 Java mapper，讓 UI 文案組合可用 JVM tests 驗證，不必透過 Fragment workflow 才能覆蓋。
...
ViewBinding 已啟用，並已用於 `fragment_records.xml`、`fragment_result.xml`、`fragment_profile_settings.xml`。Daily、Question、Method、Ritual、Learn center、Hexagram detail 等畫面仍以 programmatic View 建立，`Ui` 仍是過渡 helper。尚未導入 bundled font、正式 vector/icon asset system 或 production-ready 墨洗資產。
...
`RecordsFragment`、`ResultFragment`、`ProfileSettingsFragment` 已切換到這批 layout；其餘畫面仍待分批遷移。
...
- `ResultPresentationTest`、`RecordCardPresentationTest`、`FavoriteHexagramPresentationTest`：覆蓋結果分享/變爻摘要、紀錄卡關係/變爻/日期/操作 label，以及收藏符號與 TalkBack label。
...
- `StableBetaWorkflowInstrumentedTest`：Espresso workflow tests，啟用 accessibility checks，覆蓋 onboarding → 本機 daily、占卜 → result auto-save → records、result recreate 不重複 auto-save、records 搜尋/占法篩選狀態保留、紀錄筆記編輯/單筆刪除、學習中心收藏、深色模式切換、JSON/text SAF export contracts 與 profile delete-all。
...
- `./gradlew --warning-mode all testDebugUnitTest lintDebug assembleDebug` 通過；本輪未輸出本專案可修的 deprecated Gradle feature warning。
...
部分畫面仍以 programmatic View 建立。這讓 MVP 快速落地，但也帶來以下風險：

- 版面可讀性不如 XML 或 Compose component tree。
- 複雜 responsive layout 不易維護。
- 視覺一致性仰賴 `Ui` helper，缺少正式 design system enforcement。

ViewBinding 已開啟，top bar、bottom nav、record item、settings row、empty state、result sections 與三個高風險畫面已有第一批 XML foundations；但 Daily、Question、Method、Ritual、Learning、Hexagram detail 與完整 form controls 仍待分批遷移。
...
- 更細的 Fragment navigation saved-state tests。
...
3. 延續 XML/ViewBinding 路線，遷移 Daily、Question、Method、Ritual、Learning、Hexagram detail 等剩餘 programmatic 畫面。
