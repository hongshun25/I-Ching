...
- 已新增 `HexagramView` 自訂 View 與 XML/ViewBinding 過渡用 layout foundations：top bar、empty state、record item、settings row。
- 已新增 Espresso stable-beta workflow tests，覆蓋 onboarding、本機模式、占卜保存、紀錄編輯/刪除、收藏、深色模式、JSON/text SAF 匯出 contract 與 delete-all。
...
- `ui/Ui`：程式化 View 的共用 helper，包含文字、卡片、pill button、chip、top bar、bottom nav 與過渡用 view factory。
...
ViewBinding 已啟用，供後續 XML component 化使用；多數 Beta 畫面仍是 programmatic View，這是目前刻意保留的過渡狀態。`res/layout/include_top_bar.xml`、`include_empty_state.xml`、`item_record.xml`、`row_settings.xml` 已先建立為下一輪畫面搬遷基礎。
...
最近一次驗證狀態：`./gradlew testDebugUnitTest lintDebug assembleDebug assembleDebugAndroidTest` 通過，`./gradlew pixel2Api35DebugAndroidTest` 通過。JVM 測試涵蓋六十四卦 pattern mapping、之卦、簡易占法一致性、占卜 snapshot JSON、舊紀錄 fallback、NavigationArgs、Room entity mapper、Room v1 schema、repository async export callback、legacy parser、匯出 JSON/純文字、紀錄搜尋/篩選、學習中心 filters 與 presentation mapper。Instrumentation 包含 app context smoke test、Room DAO insert/update/delete-all 測試，以及 stable-beta workflow tests；managed device 名稱為 `pixel2Api35`。目前 AGP 仍會在 managed-device setup 印出 `testedAbi` 提醒，但任務可完成。
...
- 大多數畫面以 Java 程式化 View 建立；ViewBinding 已啟用，XML component foundations 已建立，但 `RecordsFragment`、`ResultFragment`、`ProfileSettingsFragment` 尚未完整搬到 XML binding。
...
- 無視覺回歸測試或截圖比對。
- Fragment/Espresso workflow tests 已有第一批 stable-beta coverage，但仍缺截圖回歸、旋轉後 result 不重複儲存、records filter state 保留與完整 SAF 寫入驗收。
