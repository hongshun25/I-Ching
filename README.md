...
最近一次驗證狀態：`./gradlew testDebugUnitTest`、`./gradlew lintDebug`、`./gradlew assembleDebug`、`./gradlew assembleDebugAndroidTest` 通過。`./gradlew connectedDebugAndroidTest` 需連接實機或 emulator。JVM 測試涵蓋六十四卦 pattern mapping、之卦、簡易占法一致性、占卜 snapshot JSON、舊紀錄 fallback、NavigationArgs、Room entity mapper、Room v1 schema、repository async export callback、legacy parser、匯出 JSON/純文字、紀錄搜尋/篩選與學習中心 filter tests。Instrumentation 目前包含 app context smoke test 與 Room DAO insert/update/delete-all 測試；需實機或 emulator 執行。
