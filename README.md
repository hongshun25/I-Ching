...
目前 app 可完成 onboarding、本機帳號登入/註冊與 Guest 略過、日期驅動每日一卦、三步驟占卜、蓍草十八變互動流程、結果頁、本卦/變爻/之卦、紀錄保存與搜尋篩選、學習中心、卦象詳情、收藏、深色模式、字體大小、預設占法、每日本機提醒、Storage Access Framework 匯出 JSON/純文字，以及刪除目前帳號紀錄。
首頁的「今日想問」是占卜提問草稿：輸入後點「開始占卜」會預填 Step 1，但不會另存每日筆記或改變資料模型。
...
- 設定與收藏保存在 account-scoped app-private `SharedPreferences`；包含 dark mode、reduce motion、auto save、字體大小、預設占法、每日提醒、收藏。onboarding completion 仍是裝置層級。
- 每日提醒是本機 `AlarmManager` + notification channel；Android 13+ 需使用者允許 `POST_NOTIFICATIONS`，並透過 `RECEIVE_BOOT_COMPLETED` 在重開機後恢復已啟用的提醒。
...
- `NavigationChrome` 統一處理 top-level toolbar / bottom nav 的 system bar inset；top-level toolbar 不顯示無功能 hamburger，bottom nav 以 icon/label tint 表示 active state。
...
- `./gradlew pixel2Api35DebugAndroidTest` 通過 22/22；AGP 仍會印出 `testedAbi` setup 提醒。
