...
public class HexagramDetailFragment extends Fragment {
    ...
    public View onCreateView(@NonNull android.view.LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        page.addView(Ui.topBar(requireContext(), "←", v -> requireActivity().onBackPressed(), activity.settings().isFavorite(number) ? "♥" : "♡", v -> { activity.settings().toggleFavorite(number); activity.showHexagramDetail(number); }), new LinearLayout.LayoutParams(-1, -2));
